...
<a href="/agente/index.php" class="inline-block mb-4 font-mono text-xs uppercase tracking-wide text-slate-500 hover:text-sky-700">
    ← Voltar
</a>
...
<div class="bg-white border border-slate-200 border-l-2 border-dashed border-l-slate-400 rounded p-6">
    ...
    <form method="POST" id="form-status" class="mt-6 flex items-center gap-3 border-t border-slate-200 pt-4">
        <label class="font-mono text-xs uppercase tracking-wide text-slate-500" for="status">Status</label>
        <select name="status" id="status" class="border border-slate-300 rounded px-3 py-2">
        ...
        <button type="submit" id="btn-atualizar-status"
...
<div id="modal-confirmar-status" class="hidden fixed inset-0 z-50 items-center justify-center bg-slate-900/50 p-4" aria-hidden="true">
    <div role="dialog" aria-modal="true" aria-labelledby="modal-confirmar-status-titulo"
         class="bg-white border border-slate-200 rounded-none p-6 w-full max-w-sm">
        <div id="modal-confirmar-status-titulo" class="font-mono text-xs uppercase tracking-wide text-slate-500">
            Confirmar Atualização
        </div>
        <p class="mt-3 text-slate-900">
            Marcar este chamado como <span id="modal-confirmar-status-valor" class="font-medium"></span>?
        </p>
        <div class="mt-6 flex justify-end gap-3">
            <button type="button" id="modal-confirmar-status-cancelar"
